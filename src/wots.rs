...
pub fn wots_tw_sign<S: HashSuite>(
    ...
    for (i, &index) in idx.iter().enumerate() {
        adrs.set_type(SL_WOTS_TW_PRF)
            ...
            .set_payload2(0);
        let sk = S::prf(pk_seed, sk_seed, adrs);
        let v = chain::<S>(sk, 0, index, pk_seed, adrs, SL_WOTS_TW_HASH);
...
pub fn wots_tw_pubkey_from_sig<S: HashSuite>(
    ...
    for (i, &index) in idx.iter().enumerate() {
        ...
        let end = chain::<S>(node, index, steps, pk_seed, adrs, SL_WOTS_TW_HASH);
        chains.extend_from_slice(&end);
    }
    adrs.set_type(SL_WOTS_TW_PK).zero_payload12();
    S::t(pk_seed, adrs, &[&chains])
...
pub fn wots_c_sign<S: HashSuite>(
    ...
    for (i, &index) in idx.iter().enumerate() {
        ...
        let sk = S::prf(pk_seed, sk_seed, adrs);
        adrs.zero_payload0();
        let v = chain::<S>(sk, 0, index, pk_seed, adrs, SF_WOTS_C_HASH);
        sig.extend_from_slice(&v);
    }
    Some(sig)
}
...
pub fn wots_c_pubkey_from_sig<S: HashSuite>(
    ...
    let idx = wots_c_map_digest::<S>(pk_seed, digest, adrs, counter)?;
    let mut chains = Vec::with_capacity(WOTS_C_CHAINS_SIZE);
    adrs.zero_payload0();
    for (i, &index) in idx.iter().enumerate() {
        adrs.set_payload1(i as u32);
        ...
        let end = chain::<S>(node, index, steps, pk_seed, adrs, SF_WOTS_C_HASH);
        chains.extend_from_slice(&end);
    }
    adrs.set_type(SF_WOTS_C_PK).zero_payload12();
