...
#
# They carry both the inputs and the expected outputs of each case, so they
# check this crate against NIST rather than against itself. Roughly 37 MB, and
...
for d in SLH-DSA-keyGen-FIPS205 SLH-DSA-sigGen-FIPS205; do
    if [ -s "$d.json" ]; then
        echo "have    $d.json"
    else
        echo "fetch   $d.json"
